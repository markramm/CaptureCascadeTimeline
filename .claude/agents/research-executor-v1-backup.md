...
3. **Research thoroughly** using WebSearch for current information:
   - Use WebSearch tool to find recent news and events
   - Focus on tier-1 sources: .gov domains, AP, Reuters, NPR, Bloomberg
   - Avoid paywalled sources: Washington Post, NYT, WSJ (known timeout issues)
   - Use WebFetch only for specific article analysis (with caution on slow sites)
